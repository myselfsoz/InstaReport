...
def report_profile_attack (p_targetUsername ,p_proxyItems ):
    ...
    if ('["_js_datr","'not in fbReq .text ):
        ...
        return 
    try :
        OO000O0O00O000O00 =fbReq .text .split ('["_js_datr","')[1 ].split ('",')[0 ]
    except :
        print_error ("Connection error! (CookieParsingError)")
        return 
    _cookieTopst ={"_js_datr":OO000O0O00O000O00 }
    try :
...
if __name__=="__main__":
    proxyfilePath =input ("Enter the path to your proxy list : ") 
    parsedProxyFromFile =parse_proxy_file (proxyfilePath )
